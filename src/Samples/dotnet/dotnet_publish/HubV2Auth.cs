...
{
    public class HubV2Auth
    {
        string HostName, DeviceId, SharedAccessKey, expiryString;


        public HubV2Auth(string connectionString)
        {
            var expiry = DateTimeOffset.UtcNow.AddMinutes(60);
            this.expiryString = expiry.ToUnixTimeMilliseconds().ToString();
        ...
        public  string GetUserName()
        {
            string username = $"av=2021-06-30-preview&" +
                   ...
                   $"did={this.DeviceId}&" +
                   $"am=SAS&" +
                   $"se={this.expiryString}&" +
                   $"ca=mqttbrokerE2Etests";

            return username;
        }

        public byte[] BuildSasToken()
        {
